...
class Scrapper_boat24(Scrapper):
    ...
    def __get_links_to_ad_details(self, num_of_pages=None):
        ...
        while True:
            try:
                ...
                print(
                    f"Exception occured during getting link to page with details: {e}"
                )

            page_number += 1
            if (num_of_pages != None and page_number > num_of_pages) or (
    ...
    def __get_ad_details(self, ad_details_page_url):
        ...
        for tag in views_tags:
            if "Views" in tag.text:
                ...
                try:
                    boat_info["views"] = (
                        int(boat_info["views"]) - 1
                    )  # -1 because each time we can the page, the views counter increases
                except:
                    boat_info["views"] = None
    ...
    def get_ads(self, num_of_pages=None):
        ...
        if is_file_exist(self.m_cache_ads) == True:
            ...
            save_json_data(self.m_cache_details_links_filepath, ads)
            save_json_data(self.m_cache_ads, tmp)
        print("Ads are parsed")

        result = []
        for i in tmp:
            tmp_ad = json_to_ad(i)
            if tmp_ad.favorites_count == "one":
                tmp_ad.favorites_count = 1
            if tmp_ad.favorites_count:
                tmp_ad.favorites_count = int(tmp_ad.favorites_count)
            else:
                tmp_ad.favorites_coun = 0
            if tmp_ad.name != None:
